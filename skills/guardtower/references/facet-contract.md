...
- **Report-only.** Given the change under review, a facet returns a short, ordered, self-contained
  list of findings — capped and floored — with a durable record written to its artifact. It never
  edits code.
...
- **The workflow, in order.** Each facet runs the same four steps; its file fills in steps 1–2:
  1. **Relevance gate — first, before any lens work** (hard-stops.md §1). Does the change touch this
     ...
     having spent almost nothing, and write an artifact recording the skip. The gate is deliberately
  ...
  4. **Write the artifact and return** per the Finding schema below, to `findings.md`.
